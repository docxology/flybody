#!/usr/bin/env python3
"""
Deploy an agent on the walk imitation task.
This script provides a simplified interface for training and evaluating agents
specifically on the walk imitation task.
"""
...
from flybody.tasks.walk_imitation import WalkImitationTask
from acme import specs

def main():
    parser = argparse.ArgumentParser(description="Train and evaluate agents on walk imitation task")
    ...
    parser.add_argument("--trajectory", type=str, default=None,
                      help="Optional path to specific walking trajectory file")
    ...
    run_name = args.output_name or f"walk_imitation_{args.mode}"
    ...
    env = create_environment('walk_imitation', **env_kwargs)
    env_spec = specs.make_environment_spec(env)
    
    print(f"Created walk imitation environment with action space: {env_spec.actions.shape}")
    
    # Walking-specific agent configuration - larger network due to higher dimensional action space
    agent_config = {
        'hidden_layer_sizes': (512, 512, 256),  # Larger network for walking (59-dim action space)
        'learning_rate': 1e-4,
        'discount': 0.99,
        'batch_size': 256,
    ...
    if args.mode == "train":
        # Walk-specific training configuration
        train_config = {
        ...
        print(f"Training agent on walk imitation task for {args.episodes} episodes...")
        ...
        print(f"Deploying agent on walk imitation task...")
